...
def _generate_combinations_with_rings(rings):
    """Generate combinations of bead positions while respecting ring constraints."""
    for ring in rings:
        min_beads = max(2, len(ring) // 4)
        max_beads = (len(ring) + ring_connections) // 2

def _generate_ring_combinations(ring, ring_connections):
    """Generate combinations of bead positions for a single ring."""
    n_atoms = len(ring)
    min_beads = max(2, n_atoms // 4)
    max_beads = (n_atoms + ring_connections) // 2
    for num_beads in range(min_beads, max_beads + 1):
        for combination in itertools.combinations(ring, num_beads):
            yield combination
...
def find_bead_pos(
    ...
    atoms = graph["atoms"]
    atoms_and_neighbors = [a["neighbors"] + [a["idx"]] for a in atoms]
    # print(list_heavy_atoms)
    # print(atoms_and_neighbors)
    non_terminal_atoms = [a["idx"] for a in atoms if len(a["neighbors"]) > 1]
    nt_atoms_dict = {idx: atid for idx, atid in enumerate(non_terminal_atoms)}
    nt_atoms_dict_inv = {atid: idx for idx, atid in enumerate(non_terminal_atoms)}
    ring_atoms_ids = [[nt_atoms_dict_inv[atid] for atid in ring] for ring in ring_atoms]
    ring_connectivity = []
    for ring in ring_atoms:
        ring_neighbors = [a["neighbors"] for a in atoms if a["idx"] in ring]
        ring_neighbors_flat = [item for sublist in ring_neighbors for item in sublist]
        ring_neighbors_flat = [atid for atid in ring_neighbors_flat if atid not in ring]
    ...
    if not max_beads:
        max_beads = int(n_heavy_atoms / 2.0)
    ...
    list_heavy_atoms = nt_atoms_dict.keys()
    print(len(list_heavy_atoms))
    ring_atoms = ring_atoms_ids

    list_bonds = [a["ij"] for a in graph["bonds"]]
    list_bonds = [[nt_atoms_dict_inv[a], nt_atoms_dict_inv[b]] for a, b in list_bonds if a in nt_atoms_dict_inv and b in nt_atoms_dict_inv]
    list_bonds = np.array(list_bonds, dtype=dtype)

    # ring_atoms_flat = [item for sublist in ring_atoms for item in sublist]
    for num_beads in range(min_beads, max_beads + 1):
        ...
        if n_accceptable == 0:
            ...
            continue
        logger.info("Number of Acceptable Combinations: %d", n_accceptable)

        acceptable_trials = [[nt_atoms_dict[atid] for atid in trial] for trial in acceptable_trials]
    ...
    for comb in sorted_combs[:10000:100]:
        logger.info("Combination: %s, Energy: %f", comb[0], comb[1])
