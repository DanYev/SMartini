...
PLOT_COLUMNS = 6
HIST_BINS_LINEAR = 20
HIST_BINS_DIHEDRAL = 20
LINEAR_GRID_POINTS = 180
DIHEDRAL_GRID_POINTS = 180
AX_WIDTH_INCH = 3.0
AX_HEIGHT_INCH = 2.5
...
def plot_internal_coordinates(
    ...
    cache.setdefault("bonds", {})
    cache.setdefault("angles", {})
    cache.setdefault("dihedrals", {})
    ...
    if bonds_data:
        _plot_bonds(bonds_data, topo, output_file, temperature, cache=cache)
    if angles_data:
        _plot_angles(angles_data, topo, output_file, temperature, cache=cache)
    if dihedrals_data:
